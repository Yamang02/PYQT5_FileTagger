...
- **접근 방식 (개선)**:
    - **전용 "일괄 태그" 버튼 도입**: 메인 윈도우의 적절한 위치(예: 파일 목록 상단 또는 태그 패널 내)에 "일괄 태그" 또는 "디렉토리 태그" 버튼을 추가합니다. 이 버튼 클릭 시 `BatchTaggingDialog`가 열립니다.
    - **메인 메뉴바 항목 추가**: "파일(File)" 또는 "도구(Tools)" 메뉴 아래에 "일괄 태그 추가(Batch Add Tags)" 메뉴 항목을 추가합니다. 이 메뉴 항목 선택 시에도 `BatchTaggingDialog`가 열립니다.
- **다이얼로그 (`BatchTaggingDialog`) 설계 (개선)**:
    - **디렉토리 선택 필드 (고정 및 자동 채움)**:
        - `treeView_dirs`에서 특정 디렉토리를 우클릭하여 "일괄 태그 추가" 기능을 실행했을 경우, 해당 디렉토리 경로가 다이얼로그의 디렉토리 선택 필드에 **자동으로 채워지고 수정 불가능하도록 고정**됩니다.
        - 메인 윈도우의 전용 버튼이나 메뉴바를 통해 다이얼로그를 열었을 경우, 디렉토리 선택 필드는 비어 있거나 마지막으로 사용했던 경로가 표시되며, 사용자가 직접 디렉토리를 선택할 수 있도록 합니다.
    - **파일 미리보기 영역 도입**:
        - 다이얼로그 내부에 선택된 디렉토리의 파일 목록을 보여주는 `QTableView` 또는 `QListView` 형태의 미리보기 영역을 추가합니다.
        - 이 영역에는 파일 이름과 함께, 필요하다면 파일 크기, 수정 날짜 등 기본적인 정보도 함께 표시할 수 있습니다.
        - **필터링 반영**: "특정 확장자 파일만"과 같은 적용 범위 선택 옵션을 변경할 때마다 미리보기 영역의 파일 목록이 실시간으로 업데이트되어, 어떤 파일들이 최종적으로 태그 적용 대상이 되는지 사용자가 즉시 확인할 수 있도록 합니다.
        - **선택된 파일 강조**: (선택 사항) 만약 다이얼로그를 열기 전에 파일 목록에서 특정 파일들을 선택한 상태였다면, 해당 파일들이 미리보기 영역에서 강조 표시될 수 있도록 합니다.
    - **태그 입력/선택**: 기존 `TagInputWidget`을 재사용하여 태그 입력 필드, 자동 완성, 태그 칩 표시 기능을 제공합니다.
